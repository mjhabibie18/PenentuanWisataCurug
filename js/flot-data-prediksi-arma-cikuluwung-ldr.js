...
$(document).ready(function() {
  ...
  document.getElementById("avgNciku3").style.display = "none";
  var offset = 0;
  getTemp();
  function getTemp()
  {
    var xNo = new Array();
    var yTemp = new Array();
    var url = "http://localhost/rekomendasiwisata/Raspi04/api/apiGetDataJson_ldrciku.php";
    $.ajax({   
    type: "GET",    
      url: url,
      dataType: 'json',
      ...
      success: function(response){           
        for(i=0;i<response.records.length;i++){
          ...
          yTemp[i] = parseInt(response.records[i].ldr);
          console.log(yTemp[i]);                  
        }
...
function predictTemp(t, y)
{
  ...
  var plotObj = $.plot($("#flot-line-chart-ciku5"), [{
          data: dataOri,
          label: "data Original"
      }, {
          data: dataPredict,
  ...
  var plotObj2 = $.plot($("#flot-line-chart-ciku6"), [{
  ...
  vErr = nErr.toFixed(2);
  document.getElementById('avgDataciku5').innerHTML = "Rata-Rata : " +yPred;
  document.getElementById('avgDataciku6').innerHTML = "Mean Square Error : <b>" +vErr +"<b>";
  document.getElementById('avgNciku3').innerHTML = yPred;
}  
});